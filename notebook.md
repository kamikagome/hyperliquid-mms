...
This document outlines the transition plan for upgrading the MM quoting analysis from basic Python scripts to an AI-native, quant-style Jupyter Notebook approach.
...
- *Why?* This allows us to calculate **Queue Position** and **Re-quoting Latency**. How fast does MM cancel their $20k tier when the mid-price moves 5 bps? Do they front-run the funding rate ticks? Static REST API polling can't tell you that; async websockets plotted inline inside the notebook will.
...
**The AI-Native / ML Upgrade:** MM might scale sizes exponentially, but a hard limit of 5% will fracture tiers on thicker books or highly volatile pairs. We will replace this with a **1D DBSCAN or KMeans clustering algorithm** via `scikit-learn` in the notebook cells. 
...
- *Example prompt:* *"Here is a JSON representation of MM's quoting spread across 70 markets on HL. Which 3 markets show anomalous widening behavior relative to tier 1 global volume, indicating they are in 'risk-off' mode?"* 
...
- **10–15 minute bursts is far too short.** MM's re-quote cadence on BTC/ETH is sub-second, but the behaviors you actually want (inventory-driven skew, funding-tick reactions, session rotations between APAC/EU/US, volatility-regime changes) live on 15m–8h timescales. Target ≥24h continuous capture with a funding tick (every 1h on HL, not 8h — HL accrues hourly) crossing it. Budget the bandwidth: on the top 5 coins `l2Book` alone is a few MB/s.
- **Clock discipline.** Latency claims require monotonic local timestamps + HL server timestamps (use `time` response field), and NTP-synced hosts. Without this, "MM cancels in 42ms" is meaningless.
...
- **HLP is the counterparty story.** Hyperliquid's own HLP vault is the dominant passive liquidity and competes with MM. Any behavioral model that ignores `hlpLiquidatorState` / HLP positioning is missing the other side of the game.
- **Funding mechanics.** HL accrues funding **hourly**, not every 8h, with a premium + interest component. MM's skew almost certainly reacts to the premium signal before each tick. Plan has no funding data pipeline.
- **Maker rebates & volume tier.** HL's fee schedule is tiered by rolling 14-day volume. MM is at the top tier with negative maker fees. Their effective fair value is `mid ± rebate`, which shifts the "spread" metric by 0.1–0.3 bps. Account for it or your skew numbers are biased.
